...
# Replace "iris-stats.gpkg" with the exact name of the file you downloaded.
eu_data_path_candidates <- c(
  file.path(raw_dir, "iris-stats.gpkg"),
  file.path(cours_dir, "data", "iris-stats.gpkg"),
  file.path(cours_dir, "data", "processed", "iris-stats.gpkg")
)
eu_data_path <- eu_data_path_candidates[file.exists(eu_data_path_candidates)][1]

if (is.na(eu_data_path)) {
  message("Fichier 'iris-stats.gpkg' introuvable. Placez-le dans Cours/data/raw/ ou Cours/data/.")
} else {
  # Read the file (can take a few seconds if national coverage).
  ...
  if (!"CODE_IRIS" %in% colnames(eu_results)) {
    stop("La colonne CODE_IRIS (ou code_iris) est absente dans: ", eu_data_path)
  }

  # Join without geometry duplication.
  eu_data_clean <- st_drop_geometry(eu_results)
  iris_grenoble_eu <- merge(iris_grenoble, eu_data_clean, by = "CODE_IRIS", all.x = TRUE)

  # Select preferred variable for mapping, with safe fallback.
  value_col_candidates <- c("taux_participation", "tx_participation", "participation")
  value_col <- value_col_candidates[value_col_candidates %in% colnames(iris_grenoble_eu)][1]
  if (is.na(value_col)) {
    numeric_cols <- colnames(iris_grenoble_eu)[sapply(iris_grenoble_eu, is.numeric)]
    numeric_cols <- setdiff(numeric_cols, "CODE_IRIS")
    if (length(numeric_cols) == 0) {
      stop("Aucune variable numerique disponible pour la carte apres jointure.")
    }
    value_col <- numeric_cols[1]
    message("Variable 'taux_participation' absente. Utilisation de: ", value_col)
  }
  ...
  tm_shape(iris_grenoble_eu) +
    tm_polygons(
      ...
      palette = "Blues",
      alpha = 0.6,
      title = paste("Variable:", value_col)
    ) +
    tm_shape(sirene_grenoble) +
    tm_dots("enseigne1et", legend.show = FALSE, size = 0.05)
}
