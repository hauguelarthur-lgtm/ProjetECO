...
knitr::opts_knit$set(root.dir = normalizePath(cours_dir))
```
Introduction 
=====
...
install.packages(file.path(raw_dir, "opentripplanner_0.4.0.tar.gz"), repos = NULL, type = "source")
library(opentripplanner)
iris_grenoble<-st_read(file.path(processed_dir, "iris_grenoble.gpkg"))
#vérifiez bien que vous avez la derniere version du fichier, avec 22 lignes (22 points de vente)
sirene_grenoble<-st_read(file.path(processed_dir, "sirene_grenoble.gpkg"))
...
```{r}
img_candidates <- c(file.path(raw_dir, "gtfs.png"), file.path(cours_dir, "gtfs.png"), "gtfs.png")
img_path <- img_candidates[file.exists(img_candidates)][1]
if (!is.na(img_path)) {
...
path_data <- file.path(cours_dir, "data") # dossier OTP/graphes
...
```{r}
img_candidates <- c(file.path(raw_dir, "OTP_capture.png"), file.path(cours_dir, "OTP_capture.png"), "OTP_capture.png")
img_path <- img_candidates[file.exists(img_candidates)][1]
if (!is.na(img_path)) {
  knitr::include_graphics(img_path)
}
...
write.csv(data_temp2, file.path(processed_dir, "nb_pdv_iris.csv"))
```
</div>
...
```{r}
st_write(iris_grenoble, file.path(processed_dir, "iris_grenoble2.gpkg"), delete_dsn = TRUE)
