...
::: {#Block4 .collapse}
```{r,echo=FALSE}
img_candidates <- c(file.path(raw_dir, "folder.PNG"), file.path(cours_dir, "folder.PNG"), "folder.PNG")
img_path <- img_candidates[file.exists(img_candidates)][1]
if (!is.na(img_path)) {
  knitr::include_graphics(img_path, dpi = NA)
}
```
:::
...
::: {#Block15 .collapse}
```{r, results='hide'}
st_write(iris_grenoble, file.path(processed_dir, "iris_grenoble.gpkg"), delete_dsn = TRUE)
st_write(sirene_grenoble, file.path(processed_dir, "sirene_grenoble.gpkg"), delete_dsn = TRUE)
```
