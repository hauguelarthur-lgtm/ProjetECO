...
sirene_shp<-st_read("Cours/dataseed-sirene-2.shp")

iris_data <- read_excel("Cours/iris_isere.xlsx")
```
...
map_revenus <- tm_shape(iris_grenoble) +
  tm_polygons("Mediane_euro", alpha=0.5, palette="YlGnBu",
              ...
              title="Revenu Médian (€)") +
  tm_shape(sirene_grenoble) + 
  tm_dots("enseigne1et", legend.show = FALSE, size=0.05, popup.vars="enseigne1et") +
...
map_cadres <- tm_shape(iris_grenoble) +
  ...
  tm_shape(sirene_grenoble) + 
  tm_dots("enseigne1et", legend.show = FALSE, size=0.05) +
...
```{r}
# --- 1. Load the new Election/Demographic Data ---
# Replace "elections_2024.gpkg" with the exact name of the file you downloaded
eu_data_path <- file.path(data_dir, "data/iris-stats.gpkg") 

# Read the file
# Note: This might take a few seconds as it covers all of France
eu_results <- st_read(eu_data_path)

# --- 2. Inspect and Prepare for Join ---
# Check column names to find the IRIS code and interesting variables
# head(eu_results) 

# Usually, IRIS codes in these files are named "code_iris", "IRIS", or "CODE_IRIS"
# We standardize it to "CODE_IRIS" to match your existing dataset
if("code_iris" %in% colnames(eu_results)) {
  eu_results <- rename(eu_results, CODE_IRIS = code_iris)
}

# --- 3. Filter and Clean ---
# We drop the geometry from the new file because we only want the data 
# (we already have the map shapes in iris_grenoble)
eu_data_clean <- st_drop_geometry(eu_results)

# --- 4. Join with your Grenoble Map ---
# We merge based on the common "CODE_IRIS" column
iris_grenoble_eu <- merge(iris_grenoble, eu_data_clean, by="CODE_IRIS", all.x=TRUE)

# --- 5. Visualize the New Data ---
# Example: Mapping the turnout (Participation) or a specific party score
# You might need to check 'colnames(iris_grenoble_eu)' to find the exact variable names
# Common names in such files: "tx_participation", "Voix_RN", "Voix_LFI", etc.

tmap_mode("view")

# Example Map: Participation Rate (Check exact column name first!)
# Replace 'taux_participation' with the actual column name from the new file
tm_shape(iris_grenoble_eu) +
  tm_polygons("taux_participation", # <--- CHANGE THIS to the real variable name
              style = "pretty", 
              palette = "Blues", 
              alpha = 0.6, 
              title = "Participation EU 2024") +
  tm_shape(sirene_grenoble) +
  tm_dots("enseigne1et", legend.show = FALSE, size=0.05)
```
